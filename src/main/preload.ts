...
contextBridge.exposeInMainWorld('flippiVideo', {
  getServerInfo: (): Promise<{ port: number; token: string }> =>
    ipcRenderer.invoke('video:serverInfo'),
...
contextBridge.exposeInMainWorld('flippiReplayClips', {
  ...
  deleteClipVideo: (eventName: string, clipId: string) =>
    ipcRenderer.invoke('replayClips:deleteClipVideo', { eventName, clipId }),
  bulkDelete: (eventName: string, clipIds: string[]) =>
